#!/bin/sh
echo "Get BT5 Repositories for TAILS Script"
echo "(c) Thom Hastings 2013 New BSD License"
echo "works best when run w/ sudo"
echo
echo "Creating /etc/apt/sources.list.d/backtrack.list"
sudo touch /etc/apt/sources.list.d/backtrack.list
echo
...
sudo echo "deb http://all.repository.backtrack-linux.org revolution main microverse non-free testing" >> /etc/apt/sources.list.d/backtrack.list
...
sudo echo "deb http://source.repository.backtrack-linux.org revolution main microverse non-free testing" >> /etc/apt/sources.list.d/backtrack.list
echo
