...
# Install Java 64 bit
echo $RED"                      ====== Installing Java Development Kit ======"
cd /root &>/dev/null

if [ $(uname -m) == "x86_64" ] ; then
	#64 bit system
	wget -N http://download.oracle.com/otn-pub/java/jdk/7u21-b11/jdk-7u21-linux-x64.tar.gz &>/dev/null
	tar -xzvf jdk-7u21-linux-x64.tar.gz &>/dev/null
	if [ -d jdk1.7.0_21  ]; then
		mv jdk1.7.0_21 /opt &>/dev/null
		cd /opt/jdk1.7.0_21 &>/dev/null

		update-alternatives --install /usr/bin/java java /opt/jdk1.7.0_21/bin/java 1 &>/dev/null
		update-alternatives --install /usr/bin/javac javac /opt/jdk1.7.0_21/bin/javac 1 &>/dev/null
		update-alternatives --install /usr/lib/mozilla/plugins/libjavaplugin.so mozilla-javaplugin.so /opt/jdk1.7.0_21/jre/lib/amd64/libnpjp2.so 1 &>/dev/null
		update-alternatives --set java /opt/jdk1.7.0_21/bin/java &>/dev/null
		update-alternatives --set javac /opt/jdk1.7.0_21/bin/javac &>/dev/null
		update-alternatives --set mozilla-javaplugin.so /opt/jdk1.7.0_21/jre/lib/amd64/libnpjp2.so &>/dev/null
	else
	...
	if [ -d jdk1.7.0_21  ]; then
		...
		echo " ------ ERROR Installing Java 32 -jdk 1.7 "
	fi
fi
